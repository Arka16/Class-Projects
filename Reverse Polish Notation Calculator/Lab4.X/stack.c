...
/**
 * Pop an element off of a stack.   The popped value is passed out by reference
 * ,and the 
...
int StackIsFull(const struct Stack *stack) {
    ...
    return FALSE; //greater than stack size
...
int StackGetSize(const struct Stack *stack) {
    ...
    return stack->currentItemIndex + 1; //index starts at -1 so add 1
}